// Emits every route file under `app/(admin)/` from the entity route trees of one
...
//
//   the app path      `entities/<e>/routes/<rest>` is `app/(admin)/<rest>`, `api/<rest>`
//                     is `app/(admin)/api/<rest>`, and `crons/<n>.ts` is
//                     `app/(admin)/api/cron/<n>/route.ts`. Total, with no exceptions,
...
//                     in `app/(admin)/` and will NOT see through a re-export. They
...
/** Where a route file in an entity mounts under `app/(admin)/`, or null if it is not a route. */
export function appPathOf(rel) {
  const m = /^entities\/[a-z-]+\/(routes|api|crons)\/(.+)\.(ts|tsx)$/.exec(rel);
  if (!m) return null;
  const [, kind, rest, ext] = m;
  if (kind === "crons") return `app/(admin)/api/cron/${rest}/route.${ext}`;
  if (!ROUTE_FILES.has(rest.split("/").pop())) return null;
  if (kind === "api") return `app/(admin)/api/${rest}.${ext}`;
  return `app/(admin)/${rest}.${ext}`;
}
...
export const HAND_WRITTEN = new Set([
  ...
  "app/(admin)/api/auth/callback/route.ts",
...
function main() {
  ...
  const name = process.env.DEPLOYMENT ?? "bhutan-luxe";
  const check = process.argv.includes("--check");
  const files = generateMounts(root, name);

  const existing = sourceFiles(path.join(root, "app", "(admin)"))
    .map((p) => path.relative(root, p).split(path.sep).join("/"))
    .filter((p) => !HAND_WRITTEN.has(p) && !p.includes("__tests__"));
  ...
  if (check) {
    ...
    for (const p of stale) console.error(`app/(admin)/ has ${p}, which no installed entity mounts`);
