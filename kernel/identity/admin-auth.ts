...
// (kernel/data/supabase), which bypasses RLS. This gate — enforced in the admin
...
// requireTeamMember(); see kernel/identity/team-auth.ts.)
...
// accepted — and entities/assistant/lib/admin-chat/privileged.ts treats a single email address as
...
// below is unchanged.
//
// Wrapped in perRender() (React cache() where the runtime has it): the admin layout, the page, and any server component
...
// fail closed, never leak. Wrapped in perRender() so one render resolves it once.
export const canViewSensitive = perRender(async (email: string | null | undefined): Promise<boolean> => {
  const normalized = email?.trim().toLowerCase();
  if (!normalized) return false;
...
export const isSuperAdmin = perRender(async (email: string | null | undefined): Promise<boolean> => {
