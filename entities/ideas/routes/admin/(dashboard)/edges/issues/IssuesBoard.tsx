...
import { PersonSelect } from "@/entities/crm/client";
import type { PersonOption } from "@/entities/crm/client";
import { DEFAULT_ACCOUNTABLE_PERSON_ID, ISSUE_DIAGNOSES, type IssueRow } from "@/entities/org/client";
...
export function IssuesBoard({
  ...
  return (
    <>
      ...
      <DetailDrawer open={drawerOpen} onClose={() => setDrawerOpen(false)} eyebrow="Bhutan Luxe CRM" title="File an issue">
        <IssueForm
          krs={krs}
...
function IssueForm({
  ...
  const [assignee, setAssignee] = useState<string>(
    (DEFAULT_ACCOUNTABLE_PERSON_ID && teamOptions.some((p) => p.id === DEFAULT_ACCOUNTABLE_PERSON_ID) ? DEFAULT_ACCOUNTABLE_PERSON_ID : null) ?? (teamOptions[0]?.id ?? ""),
  );
