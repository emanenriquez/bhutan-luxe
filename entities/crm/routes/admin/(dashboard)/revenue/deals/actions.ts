...
import { bumpCompanyLifecycle, bumpPersonCompanies, getLead, recordTransition } from "@/entities/crm/lib/lifecycle";
...
// Pure helpers cannot live in this file: "use server" allows only async
// exports. They sit beside it in board-helpers.ts, where they are also
// reachable from a test.
import { normalizeUrl } from "./board-helpers";
import { stripPostgrestMetacharacters } from "@/kernel/data/postgrest-filter";
import { applyUsdConversion, reopenLeadAsConnected } from "./deal-helpers";
...
async function syncPersonAfterClose(dealId: string, personId: string | null, won: boolean): Promise<Result> {
  ...
  if (won) {
    ...
    if (lead) {
      const { error: delErr } = await companyOs.from("lead").delete().eq("person_id", personId);
      if (delErr) return { ok: false, error: `Deal stage saved, but retiring the lead failed: ${delErr.message}` };
  ...
  // that basis would undo a live pipeline, so the caller is told instead.
  const { count: otherActive, error: countErr } = await companyOs.from("deals").select("id", { count: "exact", head: true })
    .eq("person_id", personId)
    .in("status", ["open", "won"])
    .neq("id", dealId);
  if (countErr) return { ok: false, error: `Deal stage saved, but the lead sync failed: ${countErr.message}` };
  if ((otherActive ?? 0) > 0) return { ok: true };
...
export async function moveDealStage(
  ...
  const { data: stage, error: stageErr } = await companyOs.from("pipeline_stages").select("name, is_won, is_lost")
    .eq("id", toStageId)
    .maybeSingle();
  ...
  const updates: CompanyOsUpdate<"deals"> = { stage_id: toStageId, status, closed_at };
  ...
  if (stage.is_won && wonAmount != null) {
    ...
    // shouldn't block closing the deal. Refreshing the rate cache is
    // updateDeal's job, so this call leaves fx_rates alone.
    const { data: existing, error: existingErr } = await companyOs.from("deals").select("currency").eq("id", dealId).maybeSingle();
    if (existingErr) return { ok: false, error: existingErr.message };
    await applyUsdConversion(updates, dealId, cents, existing?.currency ?? "usd", { cacheRate: false });
  }
  ...
  const { data: deal, error } = await companyOs.from("deals").update(updates)
    .eq("id", dealId)
  ...
  if (stage.is_won || stage.is_lost) {
    const synced = await syncPersonAfterClose(dealId, deal?.person_id ?? null, stage.is_won);
    if (!synced.ok) { refresh(); return synced; }
  }
...
export async function demoteDealToLead(dealId: string, reason: string): Promise<Result> {
  ...
  const { error: aErr } = await companyOs.from("deals").update({ archived_at: new Date().toISOString(), archived_by: admin.email })
    .eq("id", dealId);
...
export async function updateDeal(dealId: string, patch: DealPatch): Promise<Result> {
  ...
  // Re-fetch the rate whenever amount or currency changes; whichever half the
  // patch left out is read back off the stored deal.
  if (updates.amount_cents !== undefined || updates.currency !== undefined) {
    ...
    if (amountCents === undefined || currency === undefined) {
      const { data: existing, error: existingErr } = await companyOs.from("deals").select("amount_cents, currency").eq("id", dealId).maybeSingle();
      if (existingErr) return { ok: false, error: existingErr.message };
      amountCents ??= existing?.amount_cents ?? 0;
      currency ??= existing?.currency ?? "usd";
    }
    await applyUsdConversion(updates, dealId, amountCents ?? 0, currency ?? "usd", { cacheRate: true });
  }
