...
import { STAGE_WON, STAGE_LOST, STAGE_NEUTRAL } from "@/kernel/ui/stageColors";
import { PageHead } from "@/kernel/ui/PageHead";
import { MetricCard } from "@/kernel/ui/MetricCard";
import { Badge } from "@/kernel/ui/Badge";
import { formatCents, formatDate, timeAgo } from "@/kernel/ui/format";
// The cockpit is a CRM screen, but it frames CRM numbers with company-os's
// chrome, dashboard maths, survey score and site analytics. All of it comes
// through that entity's door in one import.
import { compactUsd, vsPrior, monthsThisYear, MS_DAY, getAnalyticsOverview } from "@/entities/company-os";
import { getSurveyScore } from "@/entities/org";
import { Band } from "@/kernel/ui/Band";
...
import type { DealCard } from "./deals/types";
...
export default async function SalesCockpitPage() {
  ...
  let inqQuery = selectInquiries("id, subject, type, created_at, people(full_name, email)")
    .eq("status", "new_lead")
  ...
  const [
    ...
    selectPipelineStages("id, name, is_won, is_lost").order("position"),
    dealsQuery,
    selectLead("status, sla_due_at, created_at, people!person_id!inner(id, full_name, email, archived_at)")
      ...
      .limit(50),
    inqQuery,
    selectLead("person_id, people!person_id!inner(id)", { count: "exact", head: true })
      .in("status", ACTIVE_LEAD_STATUSES)
      .is("people.archived_at", null)
      .not("sla_due_at", "is", null)
      .lt("sla_due_at", nowIso),
    selectDeals("amount_usd_cents")
      .eq("status", "won")
      .is("archived_at", null)
      .gte("closed_at", "2026-01-01")
      .lt("closed_at", "2027-01-01"),
    // Revenue = non-voided invoices by invoice date + paid Stripe orders, all USD.
    selectInvoices("txn_date, amount_cents, balance_cents, status, entity").neq("status", "voided").limit(2000),
    selectOrders("created_at, amount_usd_cents, status").limit(2000),
    // All deals for the 30d funnel and 90d lead→won conversion.
    selectDeals("status, created_at, closed_at").is("archived_at", null).limit(2000),
    selectLead("created_at").gte("created_at", iso90).limit(1000),
    selectInquiries("id", { count: "exact", head: true })
      .not("type", "in", NON_SALES_INQUIRY_TYPES)
  ...
  const leads: LeadRow[] = ((leadsRes.data as unknown as LeadRawRow[] | null) ?? [])
    .flatMap((l) => (l.people ? [{ ...l, people: l.people }] : []))
    .map((l) => ({
      id: l.people.id,
  ...
  const entitySplit = (from: string, to: string) => (
    <>
      <div>Invoices {compactUsd(invoiceCash(from, to, "bhutan-luxe"))}</div>
      <div>Stripe {compactUsd(stripeCash(from, to))}</div>
    </>
  );
  ...
  return (
    <>
      ...
      <div className="u-grid-auto-md u-items-start">
        {/* ── Leads to work ── */}
