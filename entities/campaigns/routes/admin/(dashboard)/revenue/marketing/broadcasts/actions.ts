...
import { getBroadcast, resolveAudience, type BroadcastSegment } from "@/entities/campaigns/lib/broadcasts";
...
export async function sendTest(id: string, to?: string): Promise<ActionResult> {
  ...
  const rows = (people ?? []) as { id: string; email: string | null; first_name: string | null; display_name: string | null }[];
...
export async function approveBroadcast(id: string): Promise<ActionResult> {
  ...
  const { error } = await companyOs.from("email_campaigns").update({ status: "approved", approved_at: now, approved_by: admin.email, updated_at: now })
    .eq("id", id)
    .eq("status", "draft");
  ...
  await recordAudit({
    ...
    operation: "update",
    actor: admin.email,
    context: { approved: true, recipients: count, sendWindow: stamped },
  });
  refresh(id);
  return { ok: true };
}
