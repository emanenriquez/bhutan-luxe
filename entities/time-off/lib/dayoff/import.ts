...
import { insertPeople, insertTeamMembers, updateTeamMembers } from "@/kernel/identity/writes";
...
async function snap(endpoint: string, params: Record<string, unknown>, payload: unknown, dayoffId?: string) {
  ...
  const { error } = await companyOs.from("dayoff_snapshot").insert({
    endpoint,
    params: params as Json, // jsonb columns, fed by open records / `unknown`.
    dayoff_id: dayoffId ?? null,
    payload: (stripSecrets(payload) ?? {}) as Json,
  });
...
async function resolveOrCreatePerson(
  ...
  const { data: existing, error: personError } = await companyOs.from("people").select("id").eq("email", email).maybeSingle();
  if (personError) console.error("[time-off/dayoff-import] people", personError);
  if (existing) return { id: existing.id };
  const { data, error } = await insertPeople({
      email,
      full_name: name,
      is_team_member: true,
      persona: "employee",
...
async function ensureTeamMember(
  ...
  const { data: byDayoff, error: byDayoffError } = await companyOs.from("team_members").select("id").eq("dayoff_employee_id", dayoffId).maybeSingle();
  if (byDayoffError) console.error("[time-off/dayoff-import] team_members", byDayoffError);
  if (byDayoff) return { id: byDayoff.id };

  const { data: existingTm, error: existingTmError } = await companyOs.from("team_members").select("id").eq("person_id", personId).limit(1).maybeSingle();
  if (existingTmError) console.error("[time-off/dayoff-import] team_members", existingTmError);
  if (existingTm) {
    const { error } = await updateTeamMembers(link).eq("id", existingTm.id);
    return error ? { error: error.message } : { id: existingTm.id };
  }

  const { data, error } = await insertTeamMembers({ person_id: personId, status: active ? "active" : "alumni", ...link })
    .select("id")
    .single();
  ...
  try {
    ...
    for (const e of dayoffEmployees) {
      ...
      if (local) {
        ...
        const { error: uErr } = await updateTeamMembers({ dayoff_employee_id: e.EmployeeID, ...(policyId ? { leave_policy_id: policyId } : {}) })
          .eq("id", local.id);
        if (uErr) {
...
async function importEmployee(
  ...
  const { data: existing, error: existingError } = await companyOs.from("time_off").select("id, external_id, status").eq("team_member_id", teamMemberId).eq("external_source", SOURCE);
  if (existingError) console.error("[time-off/dayoff-import] time_off", existingError);
