...
// their manual adjustments and, where the policy says the Day Off figures
// stand, the opening balance the import anchored on 6 July 2026. One call
...
export async function getMemberLeave(teamMemberIds: string[], asOf = today()): Promise<Map<string, MemberLeave>> {
  ...
  const [membersRes, policies, timeOffRes, adjRes] = await Promise.all([
    ...
    companyOs
      ...
      .select("team_member_id, delta_days, effective_date, source")
      .in("team_member_id", ids),
  ]);
  ...
  for (const m of (membersRes.data ?? []) as MemberRow[]) {
    ...
    if (policy) {
      ...
      // The import wrote the Day Off remaining balance as adjustment rows all
      // dated on the anchor day. They either become the walk's starting point
      // or are ignored; they are never added on top of a from-scratch walk.
    ...
    out.set(m.id, { teamMemberId: m.id, policy, anniversaryDate, balance });
  }
  return out;
}
