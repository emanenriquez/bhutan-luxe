...
describe("time-off tables", () => {
  ...
  it("names the leave tables FS-03 moved off team", () => {
    // team kept the tables its own screens write; the four leave tables moved
    // with the code, so the entity that owns the screens owns the data.
    for (const table of ["dayoff_snapshot", "leave_adjustments", "leave_policies", "time_off"]) {
