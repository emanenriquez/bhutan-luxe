import { selectSurveyAnswers, selectSurveyResponses } from "@/entities/org/lib/reads";
...
export default async function SurveyResultsPage(props: { params: Promise<{ id: string }> }) {
  ...
  // One query for every field instead of one per field. The old per-field query
  // capped at 2,000 answers each; a single explicit cap replaces it, sized to
  // cover the 500 responses this page already caps at times a wide survey.
  const ANSWER_CAP = 20000;
  const answersByField = new Map<string, AnswerRow[]>();
  for (const f of fields) answersByField.set(f.id, []);
  if (fields.length > 0) {
    const { data: answerRows, error: answersError } = await selectSurveyAnswers("id, response_id, field_id, value, value_json")
      .in(
        "field_id",
        fields.map((f) => f.id),
      )
    ...
    for (const a of (answerRows ?? []) as AnswerRow[]) {
      answersByField.get(a.field_id)?.push(a);
    }
  }
  ...
  if (!survey.is_anonymous && personIds.length > 0) {
    const { data: dir, error: dirError } = await companyOs
      .from("team_directory")
      .select("id, person_id")
      .in("person_id", personIds);
    if (dirError) console.error("[operations/surveys] directory load failed:", dirError.message);
  ...
  const anonymous = survey.is_anonymous;
