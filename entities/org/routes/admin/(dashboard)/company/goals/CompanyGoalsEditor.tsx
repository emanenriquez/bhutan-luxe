...
import {
  ...
  type KrRow,
  type KrStatus,
} from "@/entities/org/lib/company/edges-shared";
import type { LadderedPerson, ObjectiveWithKrs } from "@/entities/org/lib/company/goal-shapes";
import { barClass, fmtValue } from "@/entities/org/ui/company/CompanyGoalsObjectives";
...
// place these are edited (the Bhutan Luxe CRM cascade board is retired). Every
                ...
                onSubmit={(d) =>
                  run(
                    () =>
                      createKr({
                        objective_id: o.id,
                        // No configured default: reuse an existing accountable person, else let the action reject.
                        accountable_person_id: DEFAULT_ACCOUNTABLE_PERSON_ID ?? Object.keys(initialsById)[0] ?? "",
                        title: d.title,
                        target_value: d.target === "" ? null : Number(d.target),
                        unit: d.unit,
                        direction: d.direction,
