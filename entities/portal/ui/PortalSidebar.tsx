...
/** The entitlement keys this client holds, as the capability set the rows are
 *  gated on. A row with no `when` is open to every portal member. */
function entitlementKeys(entitlements: PortalEntitlements): Set<string> {
  return new Set((Object.keys(entitlements) as EntitlementKey[]).filter((k) => entitlements[k]));
}

// A row this client is not entitled to stays in the sidebar as a "soon"
// placeholder rather than disappearing: the shell reads as complete, and the
// row lights up the day the entitlement arrives. That is how the portal behaved
// before the rows became contributions (RS-14), and it is the one place this
// shell differs from the team hub, whose filterNav drops the row instead.
function gateByEntitlement(sections: NavSection[], held: Set<string>): NavGroup[] {
