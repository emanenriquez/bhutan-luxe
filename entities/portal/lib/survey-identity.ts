...
export async function resolveSurveyActor(): Promise<SurveyActor | null> {
  ...
  const { data: person, error: personError } = await companyOs
    ...
    .maybeSingle();
  if (personError) console.error("[portal] people read failed:", personError.message);
  if (person) {
    const { data: memberships, error: membershipsError } = await companyOs
      ...
      .limit(1);
    if (membershipsError) console.error("[portal] team_members read failed:", membershipsError.message);
    return {
      personId: person.id,
  ...
  if (await isAdminEmail(email)) {
    const { data: byEmail, error: byEmailError } = await companyOs
      .from("people")
      .select("id, full_name")
      .eq("email", email)
      .maybeSingle();
    if (byEmailError) console.error("[portal] people read failed:", byEmailError.message);
...
export async function classifyEmail(email: string): Promise<RespondentKind> {
  ...
  const { data: person, error: personError2 } = await companyOs
    .from("people")
    .select("id, source")
    .eq("email", normalized)
    .maybeSingle();
  if (personError2) console.error("[portal] people read failed:", personError2.message);
  if (!person) return "external";

  const { data: memberships, error: membershipsError2 } = await companyOs
    .from("team_members")
    .select("id")
    .eq("person_id", person.id)
    .in("status", PORTAL_STATUSES)
    .limit(1);
  if (membershipsError2) console.error("[portal] team_members read failed:", membershipsError2.message);
