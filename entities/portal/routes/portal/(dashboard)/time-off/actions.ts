...
async function notifyMember(id: string, decision: "approved" | "rejected", deciderName: string) {
  const { data, error: notifyErr } = await selectTimeOff("start_date, end_date, team_members!team_member_id(people!person_id(email, preferred_name, full_name))")
    .eq("id", id)
    .maybeSingle();
  if (notifyErr) console.error("[portal/time-off] time_off", notifyErr);
  if (!data) return;
