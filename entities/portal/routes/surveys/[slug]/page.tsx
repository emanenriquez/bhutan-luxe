import { selectSurveyFields, selectSurveys } from "@/entities/org";
...
import type { SurveyFieldRow, SurveyRow } from "@/entities/org";
...
export async function generateMetadata(props: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const { data, error: metaErr } = await selectSurveys("name, description")
    .eq("slug", params.slug)
    .maybeSingle();
  ...
  const meta = data as { name: string; description: string | null } | null;
  return {
    title: meta ? meta.name : "Survey",
...
export default async function PublicSurveyPage(
  props: {
    ...
    searchParams?: Promise<{ cohort?: string; review?: string; t?: string }>;
  }
) {
  const searchParams = await props.searchParams;
  const params = await props.params;
  const { data, error: surveyErr } = await selectSurveys(
      "id, slug, name, description, status, is_anonymous, intro_text, thank_you_text, purpose, created_at, updated_at, archived_at",
    )
    .eq("slug", params.slug)
    .maybeSingle();
  if (surveyErr) console.error("[portal/surveys] surveys", surveyErr);
  ...
  // the visitor must be signed in and be the review row's rater, or carry the
  // token of an external reviewer's link. The review row (not the survey) is
  // the record; see lib/reviews.ts.
  if (survey.purpose === "performance_review") {
    ...
    const ctx = await getReviewRunContext(reviewId, survey.slug, token);
    if (ctx === null) {
      ...
      // the pending link again. A tokened link is never a team member's, so a
      // bad token is simply not found.
      if (token) notFound();
    ...
    const { data: reviewFieldsData, error: reviewFieldsErr } = await selectSurveyFields("id, survey_id, position, type, label, help_text, required, config")
      .eq("survey_id", survey.id)
      .order("position", { ascending: true });
    if (reviewFieldsErr) console.error("[portal/surveys] survey_fields", reviewFieldsErr);
  ...
  const [fieldsRes, actor] = await Promise.all([
    selectSurveyFields("id, survey_id, position, type, label, help_text, required, config")
      .eq("survey_id", survey.id)
      .order("position", { ascending: true }),
    resolveSurveyActor(),
  ]);
