...
import type { FieldConfig } from "@/entities/org";
...
export async function POST(req: NextRequest, props: { params: Promise<{ slug: string }> }) {
  ...
  try {
    ...
    const { data: surveyRow, error: surveyError } = await selectSurveys("id, status, archived_at")
      .eq("slug", params.slug)
      .maybeSingle();
    if (surveyError) console.error("[portal] surveys read failed:", surveyError.message);
    // org owns surveys, so the door read erases the row shape; state it here.
    const survey = surveyRow as { id: string; status: string | null; archived_at: string | null } | null;
    ...
    const { data: field, error: fieldError } = await selectSurveyFields("id, type, config")
      .eq("id", fieldId)
      .eq("survey_id", survey.id)
      .maybeSingle();
    if (fieldError) console.error("[portal] survey_fields read failed:", fieldError.message);
