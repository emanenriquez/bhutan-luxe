...
import { validateAnswer, type SurveyFieldRow } from "@/entities/org";
import { processOnboardingSubmission } from "@/entities/onboarding";
import { recordDay8Response } from "@/entities/onboarding";
...
export async function POST(req: NextRequest, props: { params: Promise<{ slug: string }> }) {
  ...
  try {
    ...
    const { data: surveyRow, error: surveyErr } = await selectSurveys("id, name, status, is_anonymous, archived_at, purpose")
      .eq("slug", params.slug)
      .maybeSingle();
    // A read failure is not a missing survey: answer 500 like the field read
    // below, so a respondent is not told their live survey does not exist.
    if (surveyErr) {
      console.error("[surveys] survey lookup failed for", params.slug, surveyErr.message);
      return NextResponse.json({ error: "Could not load the survey." }, { status: 500 });
    }
    // org owns surveys, so the door read erases the row shape; state it here.
    const surveyData = surveyRow as
      | { id: string; name: string; status: string | null; is_anonymous: boolean | null; archived_at: string | null; purpose: string | null }
      | null;
    ...
    const { data: fieldsData, error: fieldsErr } = await selectSurveyFields("id, survey_id, position, type, label, help_text, required, config")
    ...
    if (surveyData.purpose === "performance_review") {
      ...
      const token = typeof body.t === "string" && body.t.trim() ? body.t.trim() : null;
      const ctx = await getReviewRunContext(reviewId, params.slug, token);
      if (ctx === null)
      ...
      const reviewFields = visibleReviewFields(fields, ctx.review.review_type, ctx.review.rater_kind);
    ...
    // `value_json` lands in a jsonb column; the parser hands back an open
    // value, so the jsonb value type is named here rather than at the write.
    const answerRows: { field_id: string; value: string; value_json: Json }[] = [];
    for (const field of fields) {
    ...
    if (cohortRaw) {
      const { data: eventRow, error: eventRowError } = await selectEvents("slug")
        .eq("slug", cohortRaw)
        .maybeSingle();
      if (eventRowError) console.error("[portal] events read failed:", eventRowError.message);
      cohortSlug = ((eventRow as { slug: string } | null)?.slug) ?? null;
    }

    const { data: response, error: rErr } = await insertSurveyResponses({
    ...
    const { error: aErr } = await insertSurveyAnswers(
      answerRows.map((a) => ({ ...a, response_id: response.id })),
    );
    if (aErr) {
      // Don't leave a half-saved response behind.
      await deleteSurveyResponses().eq("id", response.id);
    ...
    if (surveyData.purpose === "onboarding" && personId) {
      ...
      try {
        const { warnings } = await processOnboardingSubmission({
          personId,
        ...
        // A partly-applied onboarding used to be visible only in the server log,
        // so nobody found out until the new member's record turned out wrong.
        // Ops now hears about it while the submission is still fresh. The HTTP
        // response is unchanged: this is a notification, never a failure.
        if (warnings.length > 0) {
          void notifyOps(
            `⚠️ Onboarding post-process warnings — ${who}\n${warnings.map((w) => `· ${w}`).join("\n")}`,
          );
        }
      } catch (err) {
