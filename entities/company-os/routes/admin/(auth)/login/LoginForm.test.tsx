...
function propsOf(Wrapper: () => React.JSX.Element): LoginFormProps {
...
describe("/admin/login", () => {
  ...
  it("opens on the password screen", () => {
    ...
    expect(html).toContain(">Email me a sign-in link</button>");
  });
  ...
  it("honours ?redirect= only onto our own surfaces", () => {
    const resolve = props.redirectTo as (params: URLSearchParams) => string;
    expect(resolve(new URLSearchParams(""))).toBe("/admin");
    expect(resolve(new URLSearchParams("redirect=/admin/crm"))).toBe("/admin/crm");
    expect(resolve(new URLSearchParams("redirect=https://evil.example/x"))).toBe("/admin");
  });
});
