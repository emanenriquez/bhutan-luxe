...
import { upsertInvoices } from "@/entities/finance";
...
// customer later attaches its invoices on the next sync pass.

const MAPPING_KEY: Record<QboEntity, string> = {
  "bhutan-luxe": "qbo_customer_ids",
};

// Sync only covers 2025 onward.
...
async function loadCustomerMap(entity: QboEntity): Promise<Map<string, string>> {
  ...
  if (mapErr) console.error("[company-os/qbo-invoice-sync] companies", mapErr);
