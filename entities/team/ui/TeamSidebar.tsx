...
// The capability keys a team nav row may be gated on. Coaching appears only for
// actors who coach >=1 person (coaching_profiles rows, not the manager role,
// dotted-line coaches included). Hiring follows the same idea, not the org
// "manager" role: it shows only to hiring managers, i.e. people who own a
// requisition (or admins). "manages" opens the My Team group for any of the
// three. Every route is gated server-side regardless.
function capabilitiesOf(role: TeamRole, isCoach: boolean, isHiringManager: boolean, hasClients: boolean): string[] {
  const held: string[] = [];
