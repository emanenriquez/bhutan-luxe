...
const EXCEPTIONS = [
  ...
  { when: 'Subject has no manager on file', then: 'Refused: no cycle opens without a manager, by hand or by the scheduler', heard: 'The chat reply or the drawer notice, and the routine run log' },
  ...
  { when: 'Same reviewer asked twice', then: 'No-op: the existing row&rsquo;s link is returned again', heard: 'The reply says they were already on the cycle' },
  ...
  { when: 'Reviewer submits after the manager finalized', then: 'Row still accepted; an extra reviewer&rsquo;s row closes only when they submit', heard: 'The Reviews screen, with the submitted date' },
  { when: 'Probation ends with no finalized manager review', then: 'Reminders fire every seven days for six weeks after the row opened; the Reviews screen shows it red from a week before probation ends', heard: 'Email + the Reviews screen' },
  { when: 'Notification fails to send', then: 'The row is already saved; the link still works', heard: 'The drawer notice, or the routine run log' },
]
...
const PRS = [
  ...
  },
  {
    ...
    done: 'A sent link is stamped on the review row and the drawer shows when it went out; a closed review cannot be sent.',
  },
]
    ...
    <main>
      ...
      <section className="section u-pb-8">
        <div className="container">
          ...
          <p className="section-sub u-mt-5">
            Two routines run the scheduled path. Performance reviews runs daily at 15:00 Ho Chi Minh time: it opens a
            cycle (a self-assessment row and a manager row) for anyone whose moment has arrived in the last 21 days,
            probation at start plus six weeks, mid-year at the contract anchor plus five months, renewal at the anchor
            plus eleven, emails both parties their link, then chases every unfinished row, self, manager, reviewer or
            external, with a reminder every seven days for six weeks. Someone with no manager on file is skipped and
            counted in the run log. Probation reviews runs daily at 14:00 and emails the manager and the founder once,
            exactly 14 days before a probation ends, pointing at the person&rsquo;s profile. The human path meets the
            scheduled one at the cycle: chat and the Reviews screen reuse the open cycle the scheduler created, or open
            one the same way (probation for someone on probation, otherwise an ad-hoc review for the month), and a row
            added by hand is chased by the same weekly reminder.
          </p>
        </div>
      </section>
      ...
      <section className="section">
        <div className="container">
          ...
          <p className="section-sub u-mt-3">
            The first three are the workflow; the fourth is a convenience. All four have shipped. Each names what it
            does, what you will see, and when it is done.
          </p>
          <div className="wf-info-grid">
            {PRS.map((pr) => (
