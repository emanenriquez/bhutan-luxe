...
import { ToolingSection, ModelSection } from './sections'
...
const ELEMENTS: WorkflowElement[] = [
  { name: 'Trigger', assignment: 'machine', desc: 'The calendar. A daily 07:45 routine rolls each profile’s next 1-1 forward by its cadence and keys prep, check-ins, and trends off it; an hourly routine drafts the recaps. Nothing waits on memory.' },
  ...
  { name: 'Output', assignment: 'machine', desc: 'A prep doc per cycle, a two-tier recap, a living commitments log, and a trend report per person, refreshed after each summarized 1-1.' },
  ...
  return (
    <div className="u-x-scroll u-mt-7 u-mb-2">
      ...
      >
        ...
        <text x="330" y="932" textAnchor="middle" fontSize="15" fontWeight="700" fill={ink}>Trend analysis, per person</text>
  ...
  return (
    <main>
      <WorkflowHero
        ...
        tldr="Every team member gets a biweekly 1-1 that runs as a loop, not a line. AI preps the conversation, a human holds it, and AI drafts a two-tier recap that reaches the member only after the coach reviews and publishes it. A mid-cycle check-in on the off week feeds the next prep, and a trend analysis refreshed after each 1-1 feeds the coaching focus. The AI preps and drafts; the human decides."
        meta={[
            ...
            steps={[
              {
                ...
                cadence: 'Up to four days before',
                actor: 'ai',
                actorLabel: 'Claude',
                body: (
                  <p>
                    The daily routine finds every 1-1 due within four days and writes the prep once: GROW-structured
                    openers led by the goal question, a recommended mode, one thing to listen for, a retention check,
                    and one question to avoid. It reads the person&apos;s goals, priorities, open commitments and last
                    check-in updates, OCEAN profile, recent recaps, the coach&apos;s mode splits, and any talking points
                    the member raised. The coach gets one digest per run, by Lark DM and email, with a portal link per prep.
                  </p>
                ),
              },
              {
                ...
                cadence: 'Anchor day, every two weeks by default',
                actor: 'human',
                actorLabel: 'Coach',
                body: (
                  <>
              ...
              },
              {
                ...
                body: (
                  <>
                    <p>
                      The daily routine auto-detects a new Lark Minutes recording whose title carries 1-1 and the
                      member&apos;s first name, matches it to the meeting within a day, and pulls the transcript. If
                      none is found, the coach pastes a Minutes link or raw text on the meeting. An hourly routine then
                      drafts one held 1-1 at a time, oldest first, within the last 14 days, and tells the coach by Lark
                      DM and email: a private summary for the coach (candid reads, retention signal, mode split estimate)
                      and a shared recap for the member, plus the commitments made by either side. Always commitments,
                      never tasks: they were agreed in a conversation, not assigned from above.
                    </p>
                    <p>
                      The coach edits both tiers; the shared recap reaches the member only when the coach publishes it.
                    </p>
                  </>
                ),
              },
              {
                ...
                body: (
                  <p>
                    Halfway through the cycle, if commitments are still open and no check-in has gone out since the last
                    1-1, the member gets one AI-written nudge, a Lark DM with an email twin, linking to the portal. Course
                    corrections happen in week one, and every answer feeds the next prep: the loop talking to itself.
                  </p>
                ),
              },
              {
                num: '05',
                title: 'Trends after every 1-1, feeding the coaching focus',
                cadence: 'After each summarized 1-1',
                actor: 'ai',
                actorLabel: 'Claude',
                body: (
                  <p>
                    Once a person has two summarized 1-1s, the daily routine writes a private trend report, refreshed
                    when the latest 1-1 has none for its month, and tells the coach by Lark DM and email: trajectory,
                    recurring themes, follow-through, coaching opportunities, and flags worth raising. Patterns no one
                    catches meeting to meeting become the next cycle&apos;s coaching focus.
