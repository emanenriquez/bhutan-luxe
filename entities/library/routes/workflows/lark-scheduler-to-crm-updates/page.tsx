...
const description =
  'Two local scheduled tasks bracket every sales call: one turns each external calendar booking into a CRM lead before the call, and one turns each recorded call into a complete CRM record, a drafted follow-up, and a coaching note after it. They run on the founder\'s machine, not as managed routines. Full control flow, state machine, and failure paths included.'
...
const ELEMENTS: WorkflowElement[] = [
  { name: 'Trigger', assignment: 'machine', desc: 'Time, twice a day, on the founder&rsquo;s machine. 6am catches the bookings that arrived overnight; 6pm catches the calls that happened since.' },
  ...
  return (
    <main>
      <WorkflowHero
        ...
        tldr="A prospect books a call, the call happens, and the pipeline quietly falls behind the calendar unless someone types it all in. Two scheduled tasks close that gap. They are Claude Desktop scheduled tasks on the founder's machine, not managed routines on the company's Agents page: the CRM, the call records and the sales-intelligence scorecards they write into are real product code, the scheduling is personal. This page documents the workflow the way it is designed to run: every branch, every failure path, and the state machine underneath, not just the happy path."
        meta={[
          { label: 'Source', value: 'Lark Calendar + Minutes' },
          { label: 'Cadence', value: 'Daily, 6am and 6pm, local task' },
        ...
        <div className="container">
          ...
          <FlowRail
            steps={[
              ...
              { num: '02', title: 'Lead Created', cadence: '6am, local task', actor: 'ai', actorLabel: 'Claude' },
              ...
              { num: '05', title: 'CRM Caught Up', cadence: '6pm, local task', actor: 'ai', actorLabel: 'Claude' },
          ...
          <div className="wf-info-grid">
            ...
            <div className="wf-info-card">
              ...
              <ul>
                ...
                <li>Both tasks are Claude Desktop scheduled tasks on the founder&rsquo;s machine, not Vercel crons: a missed run executes at next wake, keys make that safe</li>
                <li>Because they are personal tasks, they do not appear on Settings &rarr; Agents and leave no routine run there; the task run log is the only record</li>
              </ul>
            </div>
      ...
      <section className="section">
        <div className="container">
          ...
          <p className="wf-lead u-mt-6">
            This workflow replaced our earlier Sales Call Intelligence pipeline: same philosophy, now designed end to
            end as two scheduled tasks, and documented at the level a developer could rebuild it from. The CRM writes,
            the call records with their transcripts, and the sales-intelligence scorecards are real product code; the
            scheduling is a personal local task, not yet a managed routine. It feeds the same pipeline as{' '}
