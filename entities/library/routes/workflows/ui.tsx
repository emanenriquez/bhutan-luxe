...
import type { WorkflowStatus } from '@/entities/library/lib/workflowsData'
...
// Built: a routine on Settings → Agents runs it. Planned: designed, not yet
// scheduled. The value comes from workflowsData, never from the page's prose.
export function StatusChip({ status }: { status: WorkflowStatus }) {
  return <span className={`wf-status wf-status-${status}`}>{status === 'built' ? 'Built' : 'Planned'}</span>
}
