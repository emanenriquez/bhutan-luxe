...
export type PortalLeaveDirectoryRow = {
  ...
  // and the page falls back to the synced usedDays/totalDays pair.
  computed: { accruedDays: number; usedDays: number; remainingDays: number; pendingDays: number } | null;
...
export async function getAssignedLeaveDirectory(actor: PortalActor): Promise<PortalLeaveDirectoryRow[]> {
  ...
  const { data, error: teamDirectoryError } = await selectTeamDirectory("id, full_name, team, location, leave_policy, work_schedule, status, used_days, total_days")
    .in("id", memberIds)
    ...
    return {
      ...
      computed: b
        ? {
            ...
            usedDays: hoursToDays(b.usedHours, hpd),
