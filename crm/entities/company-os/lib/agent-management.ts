...
const CRON_META: Record<string, CronMeta> = {
  "/api/cron/check-in-reminder/": {
    ...
    description: "Tells both team chats it is time to update their cards, thirty minutes before the check-in reads the board.",
    content: ["None — it only posts"],
    apps: ["Lark"],
  },
  "/api/cron/daily-check-in/": {
    name: "Daily check-in",
  ...
  "/api/cron/broadcast-summary/": {
    name: "Broadcast summary",
    description: "Daily. Posts one Lark summary to the Marketing channel for each broadcast whose numbers have settled (72h after send), with a one-line Claude takeaway. Latches per broadcast so each is summarised once.",
    content: ["Email campaigns", "Email events"],
    apps: ["Supabase", "Anthropic", "Lark"],
  },
  "/api/cron/marketing-recap/": {
    name: "Marketing recap",
    description: "Monthly, on the 4th. Recaps the prior month's email marketing and asks Claude for content suggestions, then stores it, posts to the Marketing Lark channel, and emails the founder. Runs on the 4th so every send is past its 72h settle window.",
    content: ["Email campaigns", "Email events", "Marketing recaps"],
    apps: ["Supabase", "Anthropic", "Resend", "Lark"],
  },
  "/api/cron/marketing-weekly-pulse/": {
    name: "Marketing weekly pulse",
    description: "Weekly, Monday. Posts a facts-only 'Marketing this week' to the Marketing Lark channel: public-site traffic with a week-over-week line, posts published, and the broadcasts that went out. No AI; the monthly recap owns content suggestions.",
    content: ["Vercel Analytics", "Marketing content", "Email campaigns"],
    apps: ["Vercel Analytics", "Supabase", "Lark"],
  },
  ...
  "/api/cron/letter-weekly/": {
    name: "Letter agent: weekly",
    description: "Monday 07:00 Ho Chi Minh City. Opens next Tuesday's letter as a draft broadcast and starts the letter agent; the run parks at ready with a test in the approver's inbox. Never sends.",
    content: ["Email campaigns", "Events", "Meetings", "Journal (Notion)", "Blog assets"],
    apps: ["Supabase", "Notion", "Anthropic", "Resend", "Lark"],
  },
  "/api/cron/writer-schedule/": {
  ...
  "/api/cron/kr-agent-sync/": {
    name: "Key result sync",
    description:
      "Nightly. Refreshes the key results whose number the company's own systems can read off (today: workflows built, counted from this page's routines) and logs each value to kr_logs.",
    content: ["Key results", "Managed routines (this page)"],
    apps: ["Supabase"],
  },
  "/api/cron/htt-rescan-hours/": {
    name: "HTT: hours rescan",
    description: "On demand, never scheduled. Re-runs the hours rule over stored sessions so a rule change reaches days already on the ledger.",
    content: ["Hours ledger (man_hour_entries)"],
    apps: ["Supabase"],
  },
};
...
// Routes gated by the cron bearer and recorded as routine runs, but called by
// the app itself rather than by a schedule. They are not in vercel.json (nothing
// polls), so they are listed here to keep their runs visible beside the crons.
export const ON_DEMAND_ROUTINES: Routine[] = [
  {
    id: "/api/cron/writer-agent/",
    name: "Writer agent",
    description:
      "On demand. Runs one step of a campaign's writer run (draft, edit, SEO, exhibits, hero, links, assemble, validate) and hands the run on to the next step. Started from the campaign hub's Run the writer button; one run row per step.",
    host: "vercel",
    hostLabel: "Vercel",
    schedule: "On demand, from the campaign hub",
    content: ["Marketing campaigns", "Blog assets", "Brand profiles"],
    skill: "app/api/cron/writer-agent/route.ts",
  ...
  },
  {
    ...
    name: "Letter agent",
    description:
      "On demand. Runs one step of a broadcast's letter agent run (gather the week, pick three unsent posts, write, rotate the call to action and layout, assemble, validate and send the test) and hands the run on. Started by the Monday cron or the broadcast page's Run the letter agent button; parks at ready, never sends to the list.",
    host: "vercel",
    hostLabel: "Vercel",
    schedule: "On demand, from the broadcast page or the Monday letter cron",
    content: ["Email campaigns", "Events", "Meetings", "Journal (Notion)", "Blog assets", "Brand profiles"],
    skill: "app/api/cron/letter-agent/route.ts",
    apps: ["Supabase", "Notion", "Anthropic", "Resend", "Lark"],
  },
];
