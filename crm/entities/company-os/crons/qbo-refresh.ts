...
const ENTITIES: QboEntity[] = ["bhutan-luxe"];
...
async function keepalive(entity: QboEntity) {
  ...
  if (!result.ok) {
    await notifyOps(
      `⚠️ QuickBooks (${entity}) token refresh failed (${result.error}). Invoicing degrades to manual until reconnected: ${QBO_SETTINGS_URL}`,
    );
  ...
  if (after.connected) {
    ...
    if (daysLeft < 14) {
      await notifyOps(
        `⚠️ QuickBooks (${entity}) refresh token expires in ${Math.floor(daysLeft)} days — reconnect at ${QBO_SETTINGS_URL}`,
      );
    }
  }
