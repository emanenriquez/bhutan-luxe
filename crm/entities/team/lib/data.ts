...
import { type OrgEntry, type OpenRole } from "@/entities/org";
...
export async function getOwnLeaveSummary(actor: TeamActor): Promise<OwnLeaveSummary | null> {
  const { data, error: dataError } = await selectTeamDirectory("leave_policy, total_days, used_days")
    .eq("id", actor.teamMemberId)
    .maybeSingle();
  if (dataError) console.error("[team/data] team_directory", dataError);
  if (!data) return null;
...
export async function getOpenRoles(): Promise<OpenRole[]> {
  const { data, error: dataError } = await selectJobRequisitions("id, title, slug, location, employment_type, is_public, hiring_manager_id")
    .eq("status", "open");
  if (dataError) console.error("[team/data] job_requisitions", dataError);
