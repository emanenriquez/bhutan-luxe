...
import { resolveLeaveApprover, clientWatcherEmails } from "@/entities/time-off";
import { LEAVE_TYPES, LEAVE_TYPE_LABEL, countWorkingDays, formatDays, type LeaveType } from "@/entities/time-off";
...
export async function cancelOwnTimeOff(id: string): Promise<Result> {
  ...
  const { data: row, error: rowError } = await teamRead(actor, "time_off", "status, team_member_id")
    .eq("id", id)
    .maybeSingle();
  if (rowError) console.error("[time-off] time_off", rowError);
