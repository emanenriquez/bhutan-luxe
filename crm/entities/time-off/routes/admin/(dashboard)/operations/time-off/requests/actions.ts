...
import { LEAVE_TYPES, type LeaveType } from "@/entities/time-off";
...
async function actingTeamMemberId(email: string): Promise<string | null> {
  const { data: person, error: personErr } = await companyOs
    ...
    .maybeSingle();
  if (personErr) {
    console.error("[operations/time-off] person lookup failed:", personErr.message);
    return null;
  }
  ...
  const { data: tm, error: tmErr } = await companyOs
    ...
    .maybeSingle();
  if (tmErr) {
    console.error("[operations/time-off] team member lookup failed:", tmErr.message);
    return null;
  }
  return tm?.id ?? null;
}
