...
import { countWorkingDays, formatLeaveBalance } from "@/entities/time-off";
...
export default async function TimeOffPage() {
  ...
  const { data: approverPeople, error: approverError } = clientApproverIds.length
    ? await companyOs
        .from("people")
        .select("id, display_name, preferred_name, full_name, email")
        .in("id", clientApproverIds)
    : { data: [] as (Person & { id: string })[], error: null };
  if (approverError) console.error("[operations/time-off] approver load failed:", approverError.message);
