...
export async function submitInquiry(
  ...
  // Save to the CRM (app/(admin), same deployment). Non-blocking: emails
  // always go out even if the save fails.
  ...
  try {
    const person = await getOrCreatePerson({
      ...
      source: "bhutan-luxe.com",
    });
    if (!person.ok) {
      console.error("[inquiry-crm-person-failed]", { error: person.error, email: payload.email });
    } else {
      const groupSize = Number(payload.groupSize);
      const { data, error } = await insertInquiries({
        person_id: person.id,
        type: "trip",
        subject: [payload.tier, payload.travelWindow].filter(Boolean).join(" · ") || "Concierge inquiry",
        message: payload.notes || null,
        source: "website",
        source_site: "bhutan-luxe.com",
        status: "new_lead",
        metadata: {
          tier: payload.tier || null,
          travel_window: payload.travelWindow || null,
          group_size: Number.isFinite(groupSize) && groupSize > 0 ? groupSize : null,
        ...
        .single();
      if (error) {
        console.error("[inquiry-crm-insert-failed]", { error: error.message, email: payload.email });
      } else {
